            ...
            if (command) {
                ...
                const fakeInteraction = {
                    ...
                    options: {
                        getUser: async (name) => {
                            if (name === 'opponent' || name === 'user') {
                                ...
                                // Nếu không có @mention, thử tìm bằng User ID thuần
                                const rawId = args.find(a => /^\d{17,20}$/.test(a));
                                if (rawId) {
                                    try {
                    ...
                    editReply: async function(data) {
                        if (this._sentMessage) return this._sentMessage.edit(data);
                        this._sentMessage = await message.reply(data);
                        return this._sentMessage;
                    },
                    deferReply: async function() {
                        this._sentMessage = await message.reply({ content: '⏳ Đang xử lý...' });
                    }
                };
