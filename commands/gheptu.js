...
module.exports = {
    ...
    async execute(interaction) {
        const language = (interaction.options && interaction.options.getString('language')) || 'vi';
        let dic = global.dicData;
        if (language === 'en') dic = global.enDicData;
        ...
        while (targetWord.length < 5 || targetWord.length > 9) {
            targetWord = dic[Math.floor(Math.random() * dic.length)];
        }
