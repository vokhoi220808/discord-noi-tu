    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option => 
            option.setName('language')
                .setDescription('Ngôn ngữ nối từ')
                .setRequired(true)
                .addChoices(
    ...
    async execute(interaction) {
        ...
        if (!language || !['vi', 'en'].includes(language)) {
            return await interaction.reply({
                content: '❌ Bạn bắt buộc phải chọn ngôn ngữ! Cú pháp Prefix: `!solo [@người_chơi] vi/en` (VD: `!solo @user en` hoặc `!solo vi`).',
        ...
        if (!opponent || opponent.bot) {
            try {
                ...
                if (e.code === 11000) {
                    return await interaction.reply({ content: 'Kênh này đang có một trận đấu Solo diễn ra. Vui lòng thử lại sau!', flags: [MessageFlags.Ephemeral] });
                }
                throw e;
            }
            ...
            const embed = new EmbedBuilder()
                .setColor('#FF0000')
                .setTitle(`⚔️ THÁCH ĐẤU SOLO VỚI BOT (${langStr}) ⚔️`)
                .setDescription(`Trận đấu giữa <@${player1.id}> và **Độ Mixi** chính thức bắt đầu!\n\n⏳ **Thời gian suy nghĩ:** 30 giây/lượt\n\n🎯 **Lượt đầu tiên:** <@${player1.id}> hãy nhập một từ bất kỳ để bắt đầu!`);
        ...
        const embed = new EmbedBuilder()
            .setColor('#FFD700')
            .setTitle(`⚔️ LỜI MỜI THÁCH ĐẤU NỐI TỪ (${langStr}) ⚔️`)
        ...
        try {
            ...
            if (e.code === 11000) {
                return await interaction.editReply({ content: 'Đã có một trận Solo được tạo cùng lúc ở kênh này! Vui lòng đợi trận đó kết thúc.', embeds: [], components: [] });
            }
            throw e;
        }
